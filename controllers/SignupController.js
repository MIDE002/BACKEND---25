...
exports.signup = async (req, res) => {
  try {
    ...
    if (idType === 'student' && !email.endsWith(studentDomain)) {
      return res.status(400).json({ error: 'Invalid email domain for student account' });
    }

    if (idType === 'admin' && !email.endsWith(adminDomain)) {
      return res.status(400).json({ error: 'Invalid email domain for admin account' });
    }



    if (idType === 'hall admin' && !email.endsWith(halladminDomain)) {
      return res.status(400).json({ error: 'Invalid email domain for hall admin account' });
    }
    // // Disable registration for admin accounts
    if (idType === 'admin') {
      return res.status(400).json({ error: 'Registration not allowed for admin accounts' });
    }
